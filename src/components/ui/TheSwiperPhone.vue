...
export default {
  name: "TheSwiperPhone",
  props: ["phoneLocation", "phoneTempl"],
  mounted() {
    ...
    swiper.addEventListener("touchmove", (el) => {
      ...
      if (
        ...
        if (xDiff > 0) {
          if (this.$props.phoneTempl === false) {
            swiper.style.translate = `${xDiff - pointGet / 8}px`;
            this.$refs.delete.style.width = `${
              Math.abs(xDiff) - pointGet / 8
            }px`;
          ...
          if (
            this.$props.phoneLocation === false ||
            this.$props.phoneLocation === "false"
          ) {
            swiper.style.translate = `${xDiff + pointGet / 8}px`;
